...
#[Instructions(<<<'TEXT'
    ...
    3. Вызовите next_id с этим project_key, кодом типа и темой документа. Повтор с той же темой безопасен: вернётся тот же номер. Сервис выдаёт только номер, имя документа собирает репозиторий: номер дополняется нулями до трёх цифр — `docs/adr/adr-043-<slug>.md`, `specs/043-<slug>/`; номер от 1000 пишется целиком. Для Spec Kit передайте номер явно и остановитесь, если созданный каталог начинается не с этого трёхзначного номера.
