...
@section('content')
    ...
    <p class="muted">Код после заведения не меняется. Шаблон: <code>{number}</code> или <code>{number:0Nd}</code> обязателен, <code>{name}</code> — по желанию.</p>
    ...
    <form method="POST" action="{{ route('admin.key-types.store') }}">
        ...
        @include('admin.partials.field', ['name' => 'format_template', 'label' => 'Шаблон, например RFC-{number:03d}-{name}', 'value' => old('format_template'), 'required' => true])
