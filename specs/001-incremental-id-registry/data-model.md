...
| `token_id` | `bigint unsigned` | FK → `personal_access_tokens.id`, `ON DELETE SET NULL`, nullable — с какой машины пришёл запрос |
| `method` | `varchar(10)` | |
...
Сетевого адреса нет: журнал отвечает на вопрос «кто из коллег», и это `user_id` плюс `token_id`
...
`user_id` и `token_id` обнуляются при удалении владельца, а не блокируют его: журнал не является
реестром и не обязан переживать владельца записи.
