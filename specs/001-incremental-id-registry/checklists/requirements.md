...
- Открыт один маркер — FR-007 (нормализация темы документа при проверке повторной выдачи). Он
  передан в `/speckit-clarify` вместе с остальными вопросами, а не задан отдельно: ответ меняет
  поведение идемпотентности, и его место — в списке уточнений перед планированием.
