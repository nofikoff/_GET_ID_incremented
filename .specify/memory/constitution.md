...
Modified principles: II переопределён несовместимо — номер, снятый администратором с хвоста пары,
  выдаётся снова (spec 003, ADR-003); отсюда MAJOR. I уточнён: повтор действует, пока запись темы есть
  в реестре. Преамбула приведена к тому же.
Unchanged: III–VII.
Added sections: нет.
...
выданный номер не будет выдан второй раз, пока администратор не снял его с хвоста пары, и что повтор
того же запроса не создаст второй номер. Всё ниже защищает ровно это.
...
Ответ на повторный запрос той же тройки «проект + тип + тема», пока её запись есть в реестре:
[CLAUDE.md §Выдача номера](../../CLAUDE.md#выдача-номера).

### II. Номер не переиспользуется, кроме снятого с хвоста

Удаление и изменение записей реестра, единственное исключение — снятие последнего номера пары,
вывод сущностей из обращения:
[CLAUDE.md §Выдача номера](../../CLAUDE.md#выдача-номера),
[ADR-001](../../docs/adr/adr-001-sequence-locking.md),
[ADR-003](../../docs/adr/adr-003-withdraw-last-identifier.md).
...
**Version**: 2.0.0 | **Ratified**: 2026-09-20 | **Last Amended**: 2026-09-23
